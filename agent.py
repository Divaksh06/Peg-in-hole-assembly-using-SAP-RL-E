...
class DDQNAgent:
    ...
    def selection(self, img_seq_tensor, proprio_seq_sensor, bt=1.0):
        with torch.no_grad():
    ...
    def learn(self, batch_size):
        ...
        img_seqs = torch.tensor(img_seqs, dtype=torch.float32)
        proprio_seqs = torch.tensor(proprio_seqs, dtype=torch.float32)
        actions = torch.tensor(actions, dtype=torch.long)
        rewards = torch.tensor(rewards, dtype=torch.float32)
        next_img_seqs = torch.tensor(next_img_seqs, dtype=torch.float32)
        next_proprio_seqs = torch.tensor(next_proprio_seqs, dtype=torch.float32)
        dones = torch.tensor(dones, dtype=torch.float32)

        q_values, pred_images, pred_attention, _ = self.policy_net(img_seqs, proprio_seqs)
        ...
        loss_image = F.mse_loss(pred_images, next_img_seqs)
